...
# SECURE PAYMENT
    ...
    # ---------------------------------------------------------------
        ...
        st.markdown(
            ...
            <div class="payment-result">
                ...
                <div class="pattern-info">
                    Select your registered colour pattern in the correct order.
                </div>
            </div>
            """,
            unsafe_allow_html=True
        ...
        sequence_text = (
            ...
            if selected_colors
            else 'No colours selected'
        )

        st.markdown(
            f'<div class="pattern-sequence">'
            f'Selected: {sequence_text}'
            f'</div>',
            unsafe_allow_html=True
        )
        ...
        st.caption(
            ...
            'registered pattern is YELLOW → PURPLE → BLUE.'
        )
    ...
    # ---------------------------------------------------------------
        ...
        st.warning(
            'This transaction remains protected and has not been authorized.'
        )
    ...
    # ---------------------------------------------------------------
        ...
        st.caption(
            "Prototype transaction — no real money was transferred."
        )
    ...
    # ---------------------------------------------------------------
        ...
        st.warning(
            'Payment cancelled. No authorization was attempted.'
        )
...
# RISK ANALYSIS
    ...
    if result:
        ...
        if source == 'voice':
            ...
            st.info(
                'This risk analysis is based on the selected voice recording.'
            )

        else:

            st.info(
                'This risk analysis is based on the selected payment.'
            )
        ...
        st.info(
            'Run a payment or voice analysis before opening Risk Analysis.'
        )
...
# FRAUD DASHBOARD
    ...
    st.caption(
        'Operational fraud prevention intelligence'
    )
    ...
    try:
        ...
        if incidents:
            ...
            for index, incident in enumerate(incidents):
                ...
                date_value, time_value = split_timestamp(
                    timestamp
                )
        ...
        st.warning(
            f'Database currently unavailable: {e}'
        )
...
# INCIDENT REPORT
    ...
    st.caption(
        'Incident history, detailed risk analysis and downloadable evidence'
    )
    ...
    if not incidents:
        ...
        if not selected_id:
            ...
            st.caption(
                'Select an incident to open its complete risk analysis.'
            )

            for index, incident in enumerate(incidents):

                current_id = incident.get(
                    'incident_id',
                    'UNKNOWN'
                )
            ...
            st.caption(
                'Select an incident ID to open its complete risk analysis.'
            )

            # Table header
            header_id, header_date, header_time = st.columns(
                [4, 3, 2]
            )
            ...
            for index, incident in enumerate(incidents):
                ...
                row_id, row_date, row_time = st.columns(
                    [4, 3, 2]
                )
        ...
        # ============================================================
            ...
            if record is None:
                ...
                with score_cols[0]:
                    st.metric(
                        'Transaction Risk',
                        f"{record.get('transaction_risk', 0)}/100"
                    )

                with score_cols[1]:
                    st.metric(
                        'Voice Risk',
                        f"{record.get('voice_risk', 0)}/100"
                    )

                with score_cols[2]:
                    st.metric(
                        'Behavior Risk',
                        f"{record.get('behavior_risk', 0)}/100"
                    )
                ...
                if transaction_features:
                    ...
                    st.info(
                        'No transaction signals were stored.'
                    )
                ...
                if voice_transcript:
                    ...
                    st.info(
                        'No voice transcript was stored for this incident.'
                    )
                ...
                if behavior_signals:
                    ...
                    st.info(
                        'No behavioral signals were stored.'
                    )
                ...
                reasons = parse_stored_value(
                    record.get('detected_reasons'),
                    {}
                )
                ...
                if reasons:
                    ...
                    st.success(
                        'No additional risk reasons were stored.'
                    )
                ...
                if st.button(
                    ...
                    try:
                        ...
                        if pdf_path:
                            ...
                            st.warning(
                                'PDF generation library is not available.'
                            )
